...
/// Format outline data as JSON
pub fn format_json(data: &OutlineMap) -> Result<String, FormatError> {
    serde_json::to_string_pretty(data).map_err(FormatError::from)
}

/// Format outline data as compact JSON (no whitespace)
#[allow(dead_code)]
pub fn format_json_compact(data: &OutlineMap) -> Result<String, FormatError> {
    serde_json::to_string(data).map_err(FormatError::from)
}
...
mod tests {
    ...
    fn test_format_json_compact() {
        let data = create_test_data();
        let result = format_json_compact(&data);
        assert!(result.is_ok());
        let json = result.unwrap();
