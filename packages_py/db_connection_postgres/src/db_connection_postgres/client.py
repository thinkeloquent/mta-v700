...
async def get_async_postgres_pool(config: PostgresConfig) -> "asyncpg.Pool":
    """
    ...
    # The config.get_connection_kwargs() returns SSL context for SQLAlchemy. 
    # For raw asyncpg pool, we need to adapt.
    
    ssl_ctx = config.get_connection_kwargs().get("ssl")
...
def get_async_sqlalchemy_engine(config: PostgresConfig) -> AsyncEngine:
    """
    Create and return a SQLAlchemy AsyncEngine.
    """
    url = config.get_async_url()
    connect_args = config.get_connection_kwargs()
