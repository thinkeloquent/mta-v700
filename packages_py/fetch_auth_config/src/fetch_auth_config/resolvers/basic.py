...
def resolve_basic_auth(
    ...
    if auth_type in [AuthType.BASIC, AuthType.BASIC_TOKEN, AuthType.BASIC_EMAIL, AuthType.BASIC_EMAIL_TOKEN]:
        ...
        if auth_type in [AuthType.BASIC_EMAIL, AuthType.BASIC_EMAIL_TOKEN]:
             res = resolve_env_var_chain(primary=env_mappings.email.primary, overwrite=env_mappings.email.overwrite, fallbacks=env_mappings.email.fallbacks)
             if not res.value: raise MissingCredentialError(provider_name, 'email', res.tried)
             email = res.value
             resolved_from['email'] = res.source
        else:
             res = resolve_env_var_chain(primary=env_mappings.username.primary, overwrite=env_mappings.username.overwrite, fallbacks=env_mappings.username.fallbacks)
    ...
    if auth_type in [AuthType.BASIC, AuthType.BASIC_EMAIL]:
        res = resolve_env_var_chain(primary=env_mappings.password.primary, overwrite=env_mappings.password.overwrite, fallbacks=env_mappings.password.fallbacks)
        ...
        res = resolve_env_var_chain(primary=env_mappings.api_key.primary, overwrite=env_mappings.api_key.overwrite, fallbacks=env_mappings.api_key.fallbacks)
